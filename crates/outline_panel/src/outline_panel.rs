...
outline: struct Foo  <==== selected
  ...
  outline: baz"
                ...
                |store: &mut settings::SettingsStore, cx| {
                    store.update_user_settings(cx, |settings| {
                        settings.project.all_languages.defaults.document_symbols =
                            Some(settings::DocumentSymbols::LanguageServer);
                    });
                },
            );
        });
        ...
        outline_panel.update(cx, |outline_panel, cx| {
            ...
            assert!(
                entries.contains("lsp_only_field"),
                "Step 2: After switching to LSP, should see LSP-provided symbols including 'lsp_only_field', got:\n{entries}"
            );
            assert!(
                entries.contains("struct Foo"),
                "Step 2: LSP symbols should include kind prefix 'struct Foo', got:\n{entries}"
            );
        });
        ...
        cx.update(|_, cx| {
            settings::SettingsStore::update_global(
                ...
                |store: &mut settings::SettingsStore, cx| {
                    store.update_user_settings(cx, |settings| {
                        settings.project.all_languages.defaults.document_symbols =
                            Some(settings::DocumentSymbols::TreeSitter);
                    });
                },
            );
        });
        ...
        outline_panel.update(cx, |outline_panel, cx| {
            ...
            assert!(
                !entries.contains("lsp_only_field"),
                "Step 3: After switching back to tree-sitter, 'lsp_only_field' should be gone, got:\n{entries}"
            );
            assert_eq!(
                entries,
                indoc!(
