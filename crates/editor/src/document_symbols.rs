...
mod tests {
    ...
    async fn test_lsp_document_symbols_fetches_when_enabled(cx: &mut TestAppContext) {
        ...
        update_test_language_settings(cx, |settings| {
            settings.defaults.document_symbols = Some(DocumentSymbols::LanguageServer);
        });
    ...
    async fn test_lsp_document_symbols_nested(cx: &mut TestAppContext) {
        ...
        cx.update_editor(|editor, _window, _cx| {
            ...
            assert_eq!(names, vec!["struct Foo", "field bar"]);
        });
    }
    ...
    async fn test_lsp_document_symbols_switch_tree_sitter_to_lsp_and_back(cx: &mut TestAppContext) {
        ...
        cx.update_editor(|editor, _window, _cx| {
            // LSP returns "lsp_main_symbol" — different from tree-sitter's "fn main"
            if let Some(symbols) = &editor.outline_symbols {
                let names: Vec<&str> = symbols.1.iter().map(|s| s.text.as_str()).collect();
                assert!(
                    names.iter().any(|n| n.contains("lsp_main_symbol")),
                    "After switching to LSP, should see LSP symbols, got: {names:?}"
                );
            }
        });
        ...
        update_test_language_settings(&mut cx.cx.cx, |settings| {
            settings.defaults.document_symbols = Some(DocumentSymbols::TreeSitter);
        });
        cx.run_until_parked();
        ...
        cx.update_editor(|editor, _window, _cx| {
            if let Some(symbols) = &editor.outline_symbols {
                ...
                assert!(
                    ...
                    "After enabling LSP, should see LSP symbols, got: {names:?}"
                );
            }
        });
    }
